...
//			user.show_message(text("\red Warning: Unknown pathogen detected in subject's blood."))
	...
	if(ishuman(M))
		...
		if(H.pulse < 40 || H.pulse > 120)
			user.show_message("\blue Subject's pulse: <font color='red'>[H.get_pulse(GETPULSE_TOOL)] bpm.</font>")
		else
			user.show_message("\blue Subject's pulse: <font color='blue'>[H.get_pulse(GETPULSE_TOOL)] bpm.</font>")
